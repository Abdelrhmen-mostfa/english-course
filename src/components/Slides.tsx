...
export const WelcomeSlide = () => (
  <div className="flex flex-col items-center justify-center h-full space-y-8 text-center p-8 relative overflow-hidden">
    ...
    <motion.div variants={itemVariants} className="text-8xl mb-4">🌟👩‍🏫👧👦🌟</motion.div>
    <motion.h1 variants={itemVariants} className="text-6xl font-display text-indigo-700 font-bold drop-shadow-md">
      Welcome to Our English Lesson!
    </motion.h1>
    <motion.h2 variants={itemVariants} className="text-3xl font-bold text-pink-500 bg-white/60 px-8 py-4 rounded-3xl shadow-sm">
...
export const RulesSlide = () => {
  ...
  return (
    <div className="flex flex-col h-full p-8">
      <motion.h1 variants={itemVariants} className="text-5xl font-display text-indigo-700 text-center mb-10 flex items-center justify-center gap-4">
        📚 Class Rules
      </motion.h1>
      <motion.div variants={containerVariants} className="grid grid-cols-2 md:grid-cols-3 gap-8 flex-1 content-center max-w-6xl mx-auto w-full">
        {rules.map((rule, idx) => (
          <motion.div key={idx} variants={itemVariants} className={`flex flex-col items-center justify-center text-center p-8 rounded-[2rem] border-b-[8px] ${rule.color} shadow-lg transform transition hover:-translate-y-2`}>
            <rule.icon className="w-20 h-20 mb-6" />
            <h3 className="text-3xl font-bold text-slate-800">{rule.text}</h3>
...
export const WarmupSlide = () => {
  ...
  return (
    <div className="flex flex-col h-full p-8 relative">
      <motion.h1 variants={itemVariants} className="text-5xl font-display text-indigo-700 text-center mb-12 flex items-center justify-center gap-4">
        🎲 Warm-up Activity
      </motion.h1>
      
      <div className="flex flex-col items-center justify-start gap-12 flex-1">
        <motion.div variants={itemVariants} className="w-full flex flex-col items-center">
          <div className="bg-indigo-600 text-white p-8 rounded-3xl shadow-xl relative w-full max-w-4xl text-center border-4 border-indigo-300">
            <h2 className="text-5xl font-bold font-display tracking-wide">How was your last vacation?</h2>
        ...
        <motion.div variants={containerVariants} className="w-full max-w-6xl grid grid-cols-2 md:grid-cols-3 gap-8 mt-12">
          {answers.map((ans, idx) => (
            <motion.div 
              key={idx} 
              variants={itemVariants} 
              className={`${colors[idx]} text-white p-6 rounded-[2rem] rounded-tl-none shadow-lg text-3xl font-bold text-center transform transition hover:scale-105 cursor-pointer flex items-center justify-center min-h-[140px] border-b-8 border-black/10`}
            >
              "{ans}"
            </motion.div>
          ))}
  ...
  <div className="flex flex-col h-full p-8">
    <div className="flex-1 flex items-center justify-center relative">
        ...
        <div className="w-full max-w-5xl h-full max-h-[70vh] bg-slate-50 rounded-[3rem] border-[12px] border-indigo-300 shadow-2xl flex flex-col items-center justify-center p-8 relative overflow-hidden ring-8 ring-indigo-100 ring-offset-8 ring-offset-sky-50">
            ...
            <p className="text-4xl font-bold text-indigo-400 font-display">Textbook Image 1 Goes Here</p>
            <p className="text-slate-400 mt-4 font-bold text-xl">(Replace this placeholder with the provided image)</p>
        </div>
    </div>
  </div>
);
...
export const TextbookDialogueSlide = () => (
  <div className="flex flex-col h-full p-8 relative overflow-hidden">
      ...
      <motion.h1 variants={itemVariants} className="text-5xl font-display text-indigo-700 text-center mb-4 z-10 font-bold">
          Story Dialogue
      </motion.h1>
      <motion.p variants={itemVariants} className="text-3xl font-bold text-pink-500 text-center mb-8 z-10 bg-white/80 inline-block px-10 py-3 rounded-full self-center shadow-md">
      ...
      <motion.div variants={itemVariants} className="flex-1 flex items-center justify-center z-10 w-full">
          <div className="w-full max-w-6xl h-full max-h-[60vh] bg-white rounded-3xl border-8 border-yellow-400 shadow-2xl flex flex-col items-center justify-center p-8 relative shadow-yellow-200/50 shadow-[0_0_40px]">
              ...
              <p className="text-3xl font-bold text-yellow-500 font-display">Textbook Dialogue Image Goes Here</p>
              <p className="text-slate-400 mt-4 font-bold text-xl">(Replace this placeholder with the provided image)</p>
          </div>
      </motion.div>
  </div>
...
export const ReadingQuestionsSlide = () => (
  <div className="flex flex-col h-full p-8 relative">
      <motion.h1 variants={itemVariants} className="text-5xl font-display text-indigo-700 text-center mb-10 z-10 font-bold flex items-center justify-center gap-4">
      ...
      <motion.div variants={itemVariants} className="flex-1 flex items-center justify-center z-10">
          <div className="w-full max-w-5xl h-full max-h-[65vh] bg-sky-50 rounded-3xl border-8 border-white shadow-xl flex flex-col items-center justify-center p-8 relative ring-8 ring-sky-200 ring-offset-4 ring-offset-sky-50">
              <div className="absolute -top-6 -right-6 text-7xl transform rotate-12 drop-shadow-md">🤔</div>
              <div className="absolute -bottom-6 -left-6 text-7xl transform -rotate-12 drop-shadow-md">📝</div>
              <Image className="w-24 h-24 text-sky-300 mb-4" />
              <p className="text-3xl font-bold text-sky-600 font-display">Textbook Questions Image Goes Here</p>
              <p className="text-slate-400 mt-4 font-bold text-xl">(Replace this placeholder with the provided image)</p>
          </div>
      </motion.div>
  </div>
...
export const VocabularySlide = () => {
  ...
  return (
    <div className="flex flex-col h-full p-6">
      <motion.h1 variants={itemVariants} className="text-4xl font-display text-indigo-700 text-center mb-6 flex items-center justify-center gap-4 font-bold">
        🎯 Choose the Correct Picture
      </motion.h1>

      <motion.div variants={containerVariants} className="grid grid-cols-2 xl:grid-cols-4 gap-6 flex-1 max-w-7xl mx-auto w-full">
        {words.map((item, idx) => {
          const isInteracted = selectedWordIdx === idx;
          return (
            <motion.div key={idx} variants={itemVariants} className="bg-white rounded-3xl p-5 shadow-lg border-b-8 border-indigo-200 flex flex-col items-center">
              <h3 className="text-2xl font-bold text-slate-800 mb-4 bg-indigo-100 px-4 py-2 rounded-full w-full text-center border-2 border-indigo-200">{item.word}</h3>
              <div className="flex gap-4 w-full flex-1">
                {/* Option A */}
                <button 
                  onClick={() => setSelectedWordIdx(idx)}
                  className={`flex-1 rounded-2xl flex items-center justify-center text-6xl bg-gray-50 border-4 transition-all hover:bg-gray-100 hover:scale-105 active:scale-95
              ...
              {isInteracted && (
                <div className="mt-4 text-green-500 font-bold text-xl animate-bounce">Correct! ✅</div>
              )}
            </motion.div>
          );
        })}
...
export const NumbersSlide = () => {
  ...
  return (
    <div className="flex flex-col h-full p-8">
      <motion.h1 variants={itemVariants} className="text-5xl font-display text-indigo-700 text-center mb-16 flex items-center justify-center gap-4 font-bold">
        🏅 Ordinal Numbers
      </motion.h1>
      
      <motion.div variants={containerVariants} className="flex flex-wrap justify-center gap-x-10 gap-y-16 flex-1 content-center max-w-6xl mx-auto">
        {numbers.map((num, idx) => (
          <motion.div 
            key={idx} 
            variants={itemVariants} 
            whileHover={{ scale: 1.15, rotate: [-5, 5, 0] }}
            className={`w-36 h-36 rounded-full border-b-[10px] flex items-center justify-center shadow-xl ${getBadgeColor(num)} relative cursor-pointer`}
          >
            <div className="absolute -top-4 -right-2 text-3xl drop-shadow-sm">✨</div>
            <div className="absolute -bottom-5 bg-white px-4 py-2 rounded-full font-bold text-base shadow-lg border-2 border-inherit text-slate-800 whitespace-nowrap">
              Number {num.replace(/\D/g,'')}
            </div>
            <span className="text-5xl font-display font-bold drop-shadow-sm">{num}</span>
          </motion.div>
        ))}
      </motion.div>
    </div>
...
export const GameSlide = () => (
  <div className="flex flex-col h-full p-8 relative overflow-hidden">
      ...
      <motion.h1 variants={itemVariants} className="text-6xl font-display text-indigo-700 text-center mb-8 z-10 flex justify-center items-center gap-4 font-bold drop-shadow-sm">
          🎮 Game Time! 🎮
      </motion.h1>

      <motion.div variants={itemVariants} className="flex-1 flex flex-col items-center justify-center z-10 space-y-12">
          <a 
              href="https://create.kahoot.it/details/4e6165d3-f46a-469d-a883-1bd9d6828e07" 
              target="_blank" rel="noopener noreferrer"
              className="bg-purple-600 hover:bg-purple-700 text-white text-5xl font-display font-bold py-8 px-16 rounded-full shadow-2xl flex items-center gap-6 transform transition hover:scale-110 hover:-translate-y-2 border-[6px] border-purple-400"
          >
              <Play className="fill-current w-16 h-16" />
              Let's Play Kahoot!
          </a>

          <div className="bg-white p-8 rounded-[3rem] shadow-xl border-8 border-dashed border-purple-200 flex flex-col items-center justify-center bg-purple-50/50">
              <QrCode className="w-40 h-40 text-purple-300 mb-4" />
              <p className="text-purple-500 font-bold text-xl uppercase tracking-widest">Scan to Join</p>
          </div>
      </motion.div>
  </div>
);
...
export const ThanksSlide = () => (
  <div className="flex flex-col items-center justify-center h-full space-y-12 text-center p-8 relative overflow-hidden">
    ...
    <motion.h1 variants={itemVariants} className="text-8xl font-display text-indigo-700 drop-shadow-lg z-10 font-bold border-b-8 border-yellow-400 pb-4">
      Thanks for Your Time!
    </motion.h1>
    
    <motion.div variants={itemVariants} className="text-[10rem] z-10 drop-shadow-2xl">
      👋👧👦🧑‍🏫
    </motion.div>

    <motion.h2 variants={itemVariants} className="text-6xl font-bold text-pink-500 bg-white/90 px-12 py-6 rounded-full shadow-2xl z-10 border-8 border-pink-200 transform -rotate-2">
      See You Next Lesson!
    </motion.h2>
  </div>
);
